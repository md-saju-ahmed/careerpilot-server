...
/**
 * Testimonials display the user's name and role, but these values are never
 * accepted from the submission payload. Instead, they are derived from the
 * user's Profile to prevent impersonation and keep testimonial identity
 * consistent across the platform. If no profile information exists, generic
 * fallback values are used.
 */
async function resolveDisplayIdentity(userId: string, fallbackName?: string) {
  const profile = await Profile.findOne({ userId }).select("name role").lean();
...
export async function submitTestimonial(
  ...
  const { name, role } = await resolveDisplayIdentity(user.id, user.name);
