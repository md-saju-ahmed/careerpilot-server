...
│   │   ├── mongoFilterGuard.ts      # Defensive guards against accidental unscoped/collection-wide deletes
...
| `SessionReadModel` | `session` (external, Better Auth) | Session cleanup on suspension/deletion (deletes, not read-only) | Schema-less; deletes go through `.collection.deleteMany()` (native driver, bypassing Mongoose's query-casting layer) and a pre-flight matched-vs-total count check — see note below                                                                                                  |
| `AccountReadModel` | `account` (external, Better Auth) | Account cleanup on user deletion (deletes, not read-only)       | Schema-less; same `.collection.deleteMany()` + pre-flight check pattern as `SessionReadModel`                                                                                                                                                                                        |
...
**Why `SessionReadModel`/`AccountReadModel` bypass Mongoose's normal `deleteMany`:** both have deliberately empty (`{}`) schemas, since they're not owned by this app. Combined with the app-wide `mongoose.set("strictQuery", true)`, calling the regular `Model.deleteMany(filter)` on an undeclared field (e.g. `userId`) causes Mongoose to silently strip that key from the filter before it reaches MongoDB — turning a single-user-scoped delete into an unscoped one that wipes the entire collection. `src/lib/session.ts` and `admin.service.ts` avoid this by calling `.collection.deleteMany()` (the native MongoDB driver method, which skips Mongoose's casting layer entirely) and by running a pre-flight check that aborts the delete if the filter matches suspiciously close to the whole collection. `src/lib/mongoFilterGuard.ts` adds a second, independent layer of protection against the same class of incident caused by a different bug (an `undefined`-valued id silently serializing to an empty filter).
...
- **Guarded deletes on external auth collections** — `session`/`account` cleanup (on user suspension/deletion) runs through `.collection.deleteMany()` rather than Mongoose's regular query layer, plus a pre-flight check and `mongoFilterGuard.ts` assertions, to prevent a malformed or improperly-cast filter from ever degrading into a collection-wide delete (see [Database Models](#database-models)).
